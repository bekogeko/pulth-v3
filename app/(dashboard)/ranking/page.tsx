...
export const metadata: Metadata = {
    ...
    description: "See which concepts are rated most difficult. Track ranking movement and find your weakest areas to practice.",
};
import {and, asc, countDistinct, desc, eq, sql} from "drizzle-orm";
import {ArrowUpRight, BarChart3, Flame, PlayCircle, Target, Trophy, Users, ClipboardList} from "lucide-react";
...
async function getConceptRankings(userId?: string) {
    ...
    return rows.map<ConceptRanking>((row) => ({
        id: row.conceptId,
...
export default async function RankingPage() {
    ...
    const conceptRankings = await getConceptRankings(userId ?? undefined);
    const practiceConcepts = conceptRankings.filter((concept) => concept.questionCount > 0);
    ...
    const maxDifficulty = Math.max(...conceptRankings.map((concept) => getRatingValue(concept.difficultyRating)), 0);
    const mostDifficultConceptId = topConcept?.id;
    return (
        <div className="flex min-w-0 flex-1 flex-col gap-6 p-6">
            <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
                <div className="max-w-2xl space-y-1">
                    <h1 className="text-lg font-semibold">Difficult Concepts</h1>
                    <p className="text-sm text-muted-foreground">
                        See the hardest concepts by question difficulty, then jump into focused practice.
                    </p>
                </div>
            ...
            <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
                <Card>
                    <CardHeader className="pb-2">
                        ...
                        <CardTitle>{conceptRankings.length}</CardTitle>
                    </CardHeader>
                </Card>
                <Card>
                    <CardHeader className="pb-2">
                    ...
                    <CardContent>
                        <div className="overflow-hidden rounded-lg border border-border">
                            <Table>
                                ...
                                <TableBody>
                                    {conceptRankings.map((concept, index) => {
                    ...
                    <CardContent className="pt-6">
                        <div className="rounded-lg border border-dashed border-border p-8 text-center">
                            ...
                            <p className="mt-1 text-sm text-muted-foreground">
                                Rankings appear after concepts and practice questions are added.
                            </p>
                        </div>
                    </CardContent>
                </Card>
